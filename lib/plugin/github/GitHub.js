...
class GitHub extends Release {
  ...
  async init() {
    ...
    if (!(await this.isCollaborator())) {
      const { repository } = this.config.getContext('repo');
  ...
  get client() {
    ...
    const host = this.options.host || this.config.getContext('repo.host');
  ...
  getReleaseUrlFallback(tagName) {
    const { host, repository } = this.config.getContext('repo');
